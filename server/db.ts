...
// Lazily create the drizzle instance with mysql2 pool
...
export async function getCoachingProfile(userId: number) {
  console.log(`[getCoachingProfile] Looking up profile for userId: ${userId}`);
  const db = await getDb();
  if (!db) {
    console.log(`[getCoachingProfile] No database connection`);
    return null;
  }
  ...
  try {
    ...
    console.log(`[getCoachingProfile] Query returned ${results.length} results`);
    
    if (results.length === 0) return null;
    
    const profile = results[0];
    console.log(`[getCoachingProfile] Profile ID: ${profile.id}`);
    ...
    try {
      ...
      console.error(`[getCoachingProfile] Failed to parse goals:`, e);
    }
    
    try {
      pressures = profile.pressures ? JSON.parse(profile.pressures as string) : [];
    } catch (e) {
      console.error(`[getCoachingProfile] Failed to parse pressures:`, e);
    }
    
    try {
      challenges = profile.challenges ? JSON.parse(profile.challenges as string) : [];
    } catch (e) {
      console.error(`[getCoachingProfile] Failed to parse challenges:`, e);
    }
    ...
    console.error(`[getCoachingProfile] Error:`, error);
    throw error;
  }
}
