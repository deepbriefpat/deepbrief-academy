...
export async function sendSessionSummaryEmail(sessionId: number): Promise<boolean> {
  ...
  try {
    ...
    // Save summary to session record in database
    console.log(`[SessionSummary] Saving summary to session ${sessionId} in database`);
    await db
      .update(coachingSessions)
      .set({ summary: JSON.stringify(summaryData) })
      .where(eq(coachingSessions.id, sessionId));

    console.log(`[SessionSummary] ✅ Summary saved to session ${sessionId}`);

    // Send notification with summary preview
    const notificationContent = `Session completed with ${summaryData.keyThemes.length} key themes and ${summaryData.commitments.length} commitments.\n\nKey Themes:\n${summaryData.keyThemes.map((t, i) => `${i + 1}. ${t}`).join('\n')}\n\nView full summary in Session History.`;
    
    console.log(`[SessionSummary] Sending notification to owner`);
    const notificationSent = await notifyOwner({
      title: "Coaching Session Summary Ready",
    ...
    if (notificationSent) {
      ...
      console.warn(`[SessionSummary] ⚠️ Notification failed, but summary is saved to session history`);
    }

    return true; // Always return true since summary is saved to database
